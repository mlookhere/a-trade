...
use gex_strategy::{
    ...
    GexLevelRole, GexNormalizationError, GexScope, GexSign, MarketSnapshot, MarketSnapshotInput,
    OperatingMode, Playbook, RawGexLevel, ResearchGammaRegimeConfig, ResearchRegimeError,
    RuleProvenance, Session, SnapshotError, classify_research_gamma_regime, normalize_gex_levels,
};
...
fn spec_8_invalid_numeric_or_scope_input_fails_closed() {
    ...
    assert_eq!(
        normalize_gex_levels(500.0, &[raw(f64::NAN, 1.0, GexScope::Aggregate)]),
        Err(GexNormalizationError::InvalidStrike)
    );
    assert_eq!(
        normalize_gex_levels(500.0, &[raw(500.0, f64::INFINITY, GexScope::Aggregate)]),
        Err(GexNormalizationError::InvalidGexValue)
    );
    assert_eq!(
        normalize_gex_levels(500.0, &[raw(500.0, 1.0, GexScope::Expiry(" ".to_owned()))]),
...
fn specs_6_28_snapshot_validates_identity_time_and_data_age() {
    let snapshot =
        MarketSnapshot::build(snapshot_input(), &[raw(510.0, 10.0, GexScope::Aggregate)]).unwrap();
...
fn classify(levels: &[RawGexLevel], scope: GexScope, mixed_share_threshold: f64) -> GammaRegime {
...
fn spec_10_research_classifier_covers_positive_negative_mixed_and_unknown() {
    ...
    assert_eq!(
        classify(
            &[raw(505.0, 10.0, GexScope::Expiry("2026-09-18".to_owned()),)],
            GexScope::Aggregate,
            0.20,
        ),
        GammaRegime::Unknown
...
fn specs_7_10_research_classifier_does_not_cross_contaminate_scopes() {
    ...
    assert_eq!(classify(&raw_levels, expiry, 0.20), GammaRegime::Negative);
}

#[test]
fn specs_10_34_classifier_requires_explicit_valid_configuration() {
    let normalized = normalize_gex_levels(500.0, &[raw(505.0, 10.0, GexScope::Aggregate)]).unwrap();
...
fn specs_2_11_19_30_foundational_contract_enums_remain_explicit() {
    ...
    assert_ne!(
        Playbook::PositiveGammaMagnet,
        Playbook::NegativeGammaExpansion
    );
